...
\author{\sffamily\normalsize Patrik Drazic\\[-1pt]\small\textit{Technical Note}}
...
\begin{tikzpicture}[
  ...
  cloud/.style={draw=accent,thick,rounded corners=8pt,fill=accent!8,
              inner sep=4pt,minimum height=18pt,minimum width=28mm},
  hub/.style={draw=accent,thick,rounded corners=2pt,minimum height=18pt,
              fill=accent!8,inner sep=3pt},
  pi/.style={draw=accent,thick,rounded corners=4pt,fill=white,inner sep=6pt,
             minimum width=70mm},
  sub/.style={draw=accent!50,thin,rounded corners=2pt,fill=accent!4,
              minimum height=14pt,inner sep=2pt,font=\sffamily\scriptsize},
  device/.style={draw=accent!70,thick,rounded corners=2pt,
              fill=white,minimum width=18mm,minimum height=14pt,
              inner sep=2pt,font=\sffamily\scriptsize},
]

% --- top row: laptop, cloud
\node[box] (laptop) {Operator laptop};
\node[cloud,right=18mm of laptop] (ts) {Tailscale tailnet\\\scriptsize(MagicDNS, NAT-traversing)};

% --- pi block
\node[pi,below=14mm of ts] (pi) {};
\node[anchor=north] at (pi.north) {\textbf{Raspberry Pi Zero 2 W}};
\node[sub,below=10pt of pi.north,text width=32mm] (svc1)
   {\texttt{tailscaled} \,+\, \texttt{sshd}};
\node[sub,right=4pt of svc1,text width=32mm] (svc2)
   {\texttt{sl-*} CLI wrappers};
\node[sub,below=2pt of svc1,text width=32mm] (sys1)
   {\texttt{udev} rule\\\texttt{99-pi-monitor.rules}};
\node[sub,right=4pt of sys1,text width=32mm] (sys2)
   {\texttt{uart-logger@.service}\\(systemd template)};
\node[sub,below=2pt of sys1,text width=68mm,minimum height=11pt] (data)
   {\texttt{cat /dev/ttyXXX | ts | tee /var/log/pi-monitor/<n>/<ts>.log}};

% --- USB hub
\node[hub,below=14mm of pi] (hub) {Powered USB hub};

% --- target row
\node[device,below=12mm of hub.south,xshift=-26mm] (stl)  {ST-Link V2};
\node[device,right=6mm of stl] (ftdi1) {FTDI 1};
\node[device,right=6mm of ftdi1] (ftdi2) {FTDI 2};
...
\draw[->,thick,accent] (ts) -- node[right,font=\scriptsize]{Wi-Fi} (pi.north -| ts);
\draw[->,thick,accent] (pi.south) -- node[right,font=\scriptsize]{USB} (hub);
\draw[->,thick,accent!70] (hub.south) -- ($(stl.north)+(0,2pt)$);
\draw[->,thick,accent!70] (hub.south) -- ($(ftdi1.north)+(0,2pt)$);
\draw[->,thick,accent!70] (hub.south) -- ($(ftdi2.north)+(0,2pt)$);
